...
func testPhase1BasicOperations(t *testing.T, suite *SubEntityIntegrationTestSuite, diagramID string) {
	...
	assert.Equal(t, "user1@example.com", receivedMsg.UserID)
	...
	assert.Equal(t, "user2@example.com", updateMsg.UserID)
	...
	assert.Equal(t, "user1@example.com", removeMsg.UserID)
...
func TestAsyncAPIMessageValidation(t *testing.T) {
	...
	t.Run("PresenterRequestMessage", func(t *testing.T) {
		...
		assert.Contains(t, string(data), "presenter@example.com")
	})
	...
	t.Run("UndoRequestMessage", func(t *testing.T) {
		...
		assert.Contains(t, string(data), "undo@example.com")
	})
}
