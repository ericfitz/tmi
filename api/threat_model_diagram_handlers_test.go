...
func TestGetThreatModelDiagrams(t *testing.T) {
	...
	// Parse response as wrapped ListDiagramsResponse
	var response ListDiagramsResponse
	err := json.Unmarshal(listW.Body.Bytes(), &response)
	require.NoError(t, err)

	// Check that we got at least one item
	assert.NotEmpty(t, response.Diagrams)
	assert.GreaterOrEqual(t, response.Total, len(response.Diagrams))
	assert.Equal(t, 20, response.Limit)
	assert.Equal(t, 0, response.Offset)

	// Check that our test item is in the list
	found := false
	for _, item := range response.Diagrams {
...
func setupThreatModelDiagramRouterWithServiceAccount(isServiceAccount bool) *gin.Engine {
	...
	r.Use(func(c *gin.Context) {
		slogging.Get().WithContext(c).Debug("[TEST DEBUG] User name: %s, Request: %s %s",
			TestFixtures.OwnerUser, c.Request.Method, c.Request.URL.Path)
		c.Set("userEmail", TestFixtures.OwnerUser)
...
func TestCreateDiagram_SetsAutoGeneratedForServiceAccount(t *testing.T) {
	...
	// Create parent threat model
	tmReqBody, _ := json.Marshal(map[string]any{
		"name":        "SA Test Threat Model",
		"description": "Test threat model for service account diagram",
	})
	tmReq, _ := http.NewRequest("POST", "/threat_models", bytes.NewBuffer(tmReqBody))
	tmReq.Header.Set("Content-Type", "application/json")
	tmW := httptest.NewRecorder()
	...
	// Create diagram as service account
	diagReqBody, _ := json.Marshal(map[string]any{
		"name":        "Service Account Diagram",
		"type":        "DFD-1.0.0",
	...
	assert.Equal(t, true, response["auto_generated"], "auto_generated should be true for service account")
}
...
func TestCreateDiagram_DoesNotSetAutoGeneratedForRegularUser(t *testing.T) {
	...
	// Create parent threat model
	tmReqBody, _ := json.Marshal(map[string]any{
		"name":        "Regular User Test Threat Model",
		"description": "Test threat model for regular user diagram",
	})
	tmReq, _ := http.NewRequest("POST", "/threat_models", bytes.NewBuffer(tmReqBody))
	tmReq.Header.Set("Content-Type", "application/json")
	tmW := httptest.NewRecorder()
	...
	// Create diagram as regular user
	diagReqBody, _ := json.Marshal(map[string]any{
		"name":        "Human Created Diagram",
		"type":        "DFD-1.0.0",
	...
	// auto_generated should be absent (nil/omitted with omitempty) for regular user
	autoGenVal, exists := response["auto_generated"]
	assert.True(t, !exists || autoGenVal == false, "auto_generated should be absent or false for regular user")
}
