...
func (s *GormThreatModelStore) getAuthorizationInternal(id string, includeDeleted bool) ([]Authorization, User, error) {
	...
	owner := User{
		...
		ProviderId:    strFromPtr(tm.Owner.ProviderUserID.Ptr()),
		DisplayName:   string(tm.Owner.Name),
...
func (s *GormThreatModelStore) convertToAPIModel(tm *models.ThreatModel) (ThreatModel, error) {
	...
	if tm.CreatedByInternalUUID != "" {
		createdBy = &User{
			...
			ProviderId:    strFromPtr(tm.CreatedBy.ProviderUserID.Ptr()),
			DisplayName:   string(tm.CreatedBy.Name),
	...
	if tm.SecurityReviewerInternalUUID.Valid && tm.SecurityReviewerInternalUUID.String != "" && tm.SecurityReviewer != nil {
		securityReviewer = &User{
			...
			ProviderId:    strFromPtr(tm.SecurityReviewer.ProviderUserID.Ptr()),
			DisplayName:   string(tm.SecurityReviewer.Name),
	...
	framework := string(tm.ThreatModelFramework)
	if framework == "" {
...
func (s *GormThreatModelStore) batchLoadAuthorizationLightweight(ids []string, ownerMap map[string]User) map[string]authWithOwner {
	...
	for _, entry := range accessEntries {
		if string(entry.SubjectType) == string(AddGroupMemberRequestSubjectTypeUser) && entry.UserInternalUUID.Valid {
			userUUIDSet[entry.UserInternalUUID.String] = true
		} else if string(entry.SubjectType) == string(AddGroupMemberRequestSubjectTypeGroup) && entry.GroupInternalUUID.Valid {
	...
	for _, entry := range accessEntries {
		...
		role := AuthorizationRole(string(entry.Role))
		...
		if string(entry.SubjectType) == string(AddGroupMemberRequestSubjectTypeUser) && entry.UserInternalUUID.Valid {
			if user, ok := userMap[entry.UserInternalUUID.String]; ok {
				...
				awo.Authorization = append(awo.Authorization, Authorization{
					...
					ProviderId:    strFromPtr(user.ProviderUserID.Ptr()),
			...
			if group, ok := groupMap[entry.GroupInternalUUID.String]; ok {
				awo.Authorization = append(awo.Authorization, Authorization{
					...
					ProviderId:    string(group.GroupName),
					DisplayName:   group.Name.Ptr(),
					Role:          role,
				})
			}
		}
...
func (s *GormThreatModelStore) Update(ctx context.Context, id string, item ThreatModel) error {
	...
	return authdb.WithRetryableGormTransaction(ctx, s.db, authdb.DefaultRetryConfig(), func(tx *gorm.DB) error {
		...
		if item.Status != nil && *item.Status != "" {
			newStatus = models.DBVarchar(*item.Status)
		}
...
func (s *GormThreatModelStore) loadThreats(threatModelID string) ([]Threat, error) {
	...
	for _, tm := range threatModels {
		...
		threats = append(threats, Threat{
			...
			CellId:        cellID,
			AssetId:       assetID,
			Priority:      tm.Priority.Ptr(),
			Mitigated:     mitigated,
			Status:        tm.Status.Ptr(),
...
func (s *GormThreatModelStore) saveAuthorizationTx(tx *gorm.DB, threatModelID string, authorization []Authorization) error {
	...
	for _, auth := range authorization {
		...
		access := models.ThreatModelAccess{
			...
			SubjectType:       models.DBVarchar(subjectTypeStr),
...
func (s *GormDiagramStore) convertToAPIDiagram(diagram *models.Diagram) (DfdDiagram, error) {
	...
	if diagram.Type.Valid && diagram.Type.String != "" {
		diagType = DfdDiagramType(diagram.Type.String)
	}
	...
	autoGenerated := diagram.AutoGenerated.Bool()
	alias := diagram.Alias
...
func (s *GormDiagramStore) CreateWithThreatModel(item DfdDiagram, threatModelID string, idSetter func(DfdDiagram, string) DfdDiagram) (DfdDiagram, error) {
	...
	// Get diagram type
	var diagType models.NullableDBVarchar
	if item.Type != "" {
		t := string(item.Type)
		diagType = models.NewNullableDBVarchar(&t)
	}
	...
	if item.AutoGenerated != nil {
		diagram.AutoGenerated = models.DBBool(*item.AutoGenerated)
	}
