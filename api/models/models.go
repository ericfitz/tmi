...
type ThreatModel struct {
	...
	Alias                        int32             `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_threat_models_alias"` // Server-assigned globally-unique integer alias
...
type Diagram struct {
	...
	Alias             int32      `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_diagrams_tm_alias,priority:2"` // Server-assigned per-(threat_model_id, type) alias
	CreatedAt         time.Time  `gorm:"not null;autoCreateTime"`
	ModifiedAt        time.Time  `gorm:"not null;autoUpdateTime"`
...
type Asset struct {
	...
	IncludeInReport DBBool            `gorm:"default:1"`
	TimmyEnabled    DBBool            `gorm:"default:1"`
	Alias           int32             `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_assets_tm_alias,priority:2"` // Server-assigned per-(threat_model_id, type) alias
...
type Threat struct {
	...
	CweID           StringArray       `gorm:""` // CWE identifiers (e.g., CWE-89)
	Cvss            CVSSArray         `gorm:""` // CVSS vector and score pairs
	Ssvc            NullableSSVC      `gorm:""` // SSVC assessment result
...
type Document struct {
	...
	Alias      int32      `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_documents_tm_alias,priority:2"` // Server-assigned per-(threat_model_id, type) alias
...
type Note struct {
	...
	TimmyEnabled    DBBool         `gorm:"default:1"`
	AutoGenerated   DBBool         `gorm:"default:0;<-:create" json:"auto_generated"`
	Alias           int32          `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_notes_tm_alias,priority:2"` // Server-assigned per-(threat_model_id, type) alias
...
type Repository struct {
	...
	IncludeInReport DBBool            `gorm:"default:1"`
	TimmyEnabled    DBBool            `gorm:"default:1"`
	Alias           int32             `gorm:"not null;default:0;<-:create;uniqueIndex:uniq_repositories_tm_alias,priority:2"` // Server-assigned per-(threat_model_id, type) alias
...
type UsabilityFeedback struct {
	...
	ClientBuild   NullableDBVarchar `gorm:"size:12"`
	UserAgent     NullableDBVarchar `gorm:"size:512"`
...
type ContentFeedback struct {
	...
	Verbatim               NullableDBText    `gorm:""`
	FalsePositiveReason    NullableDBVarchar `gorm:"size:32;index:idx_content_feedback_fp_reason"`
	FalsePositiveSubreason NullableDBVarchar `gorm:"size:40"`
	ClientID               DBVarchar         `gorm:"size:32;not null"`
	ClientVersion          NullableDBVarchar `gorm:"size:32"`
	Screenshot             NullableDBText    `gorm:""`
...
type AliasCounter struct {
	...
	NextAlias  int32     `gorm:"not null;default:1"`
}
