...
// setupNoteSubResourceHandlerWithServiceAccount creates a test router that sets isServiceAccount in the context.
func setupNoteSubResourceHandlerWithServiceAccount(isServiceAccount bool) (*gin.Engine, *MockNoteStore) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
...
// TestCreateNote_SetsAutoGeneratedForServiceAccount verifies that creating a note with a
// service-account principal sets auto_generated=true on the model before persistence.
func TestCreateNote_SetsAutoGeneratedForServiceAccount(t *testing.T) {
	r, mockStore := setupNoteSubResourceHandlerWithServiceAccount(true)

	threatModelID := testUUID1

	requestBody := map[string]any{
		"name":    "Auto-Generated Note",
		"content": "Content created by automation",
	}
	...
	require.NotNil(t, capturedNote, "note should have been captured by mock")
	require.NotNil(t, capturedNote.AutoGenerated, "AutoGenerated should be set for service account")
	assert.True(t, *capturedNote.AutoGenerated, "AutoGenerated should be true for service account")

	mockStore.AssertExpectations(t)
}

// TestCreateNote_DoesNotSetAutoGeneratedForRegularUser verifies that creating a note with a
// regular user principal leaves auto_generated unset (nil / false).
func TestCreateNote_DoesNotSetAutoGeneratedForRegularUser(t *testing.T) {
	r, mockStore := setupNoteSubResourceHandlerWithServiceAccount(false)

	threatModelID := testUUID1

	requestBody := map[string]any{
		"name":    "Human-Created Note",
		"content": "Content created by a human user",
	}
	...
	assert.Nil(t, capturedNote.AutoGenerated, "AutoGenerated should be nil for regular user")

	mockStore.AssertExpectations(t)
}

// TestPatchNote_RejectsAliasOperation verifies that a PATCH targeting /alias
...
// TestPutNote_RejectsAliasInBody verifies that a PUT request with alias in
