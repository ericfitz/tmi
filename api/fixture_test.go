...
func TestFixturesInitialization(t *testing.T) {
	...
	assert.Equal(t, TestFixtures.Diagram.Owner, d.Owner)
}
...
// TestDiagramCustomAuthRules tests the custom authorization rules for diagrams
func TestDiagramCustomAuthRules(t *testing.T) {
	// Initialize test fixtures - this will reset the stores and create fresh data
	InitTestFixtures()
	diagramID := TestFixtures.DiagramID
	...
	}`, diagramID, TestFixtures.OwnerUser, TestFixtures.WriterUser, TestFixtures.WriterUser)
	
	req, _ = http.NewRequest("PUT", "/diagrams/"+diagramID, strings.NewReader(duplicateAuth))
	req.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	ownerRouter.ServeHTTP(w, req)
	assert.Equal(t, http.StatusBadRequest, w.Code, "Request with duplicate subjects should be rejected")
	...
	}`, diagramID, newOwner, TestFixtures.WriterUser, TestFixtures.ReaderUser)
	
	req, _ = http.NewRequest("PUT", "/diagrams/"+diagramID, strings.NewReader(changeOwnerReq))
	req.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	ownerRouter.ServeHTTP(w, req)
	t.Logf("PUT Response Code: %d, Body: %s", w.Code, w.Body.String())
	...
	assert.NoError(t, err)
	assert.Equal(t, newOwner, d.Owner, "Owner should be changed to the new owner")
	...
	req, _ = http.NewRequest("PATCH", "/diagrams/"+diagramID, strings.NewReader(patchChangeOwner))
	req.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	ownerRouter.ServeHTTP(w, req)
	t.Logf("PATCH Response Code: %d, Body: %s", w.Code, w.Body.String())
