...
3. **Validation** - Spectral CLI validates against Arazzo v1.0.1 specification
...
- **[docs/reference/apis/tmi-openapi.json](tmi-openapi.json)** (1.1MB) - Complete OpenAPI 3.0.3 specification
...
npx @stoplight/spectral-cli lint docs/reference/apis/tmi.arazzo.yaml \
  --ruleset arazzo-spectral.yaml \
  --format stylish
```

**Checks**:
- Arazzo v1.0.1 specification compliance
- Workflow description requirements
- Step ID uniqueness within workflows
- TMI custom rules (OAuth prerequisites, authentication chains)

### Configuration Files

#### [arazzo-spectral.yaml](../../../arazzo-spectral.yaml)

Spectral linting configuration for Arazzo specifications:
...
This runs Spectral CLI validation against both YAML and JSON outputs using the ruleset defined in [arazzo-spectral.yaml](../../../arazzo-spectral.yaml).
...
- docs/reference/apis/tmi-openapi.json: EXISTS (1.1MB)
- docs/reference/apis/api-workflows.json: EXISTS
- docs/reference/apis/tmi.arazzo.yaml: EXISTS
- docs/reference/apis/tmi.arazzo.json: EXISTS
- docs/reference/apis/arazzo/scaffolds/base-scaffold.arazzo.yaml: EXISTS
- scripts/generate-arazzo-scaffold.sh: EXISTS
- scripts/enhance-arazzo-with-workflows.py: EXISTS (740 lines)
- scripts/validate-arazzo.py: EXISTS
- arazzo-spectral.yaml: EXISTS (not .spectral.yaml as previously documented)
- package.json: EXISTS

## Make Targets Verified:
- generate-arazzo: VERIFIED in Makefile
- arazzo-install: VERIFIED in Makefile
- arazzo-scaffold: VERIFIED in Makefile
- arazzo-enhance: VERIFIED in Makefile
- validate-arazzo: VERIFIED in Makefile
- arazzo-all: VERIFIED in Makefile

## External References Verified:
- Arazzo Specification v1.0.1: VERIFIED (https://spec.openapis.org/arazzo/latest.html)
- RFC 7636 - PKCE: VERIFIED (IETF standard)
- Redocly CLI generate-arazzo: VERIFIED (https://redocly.com/docs/cli/commands/generate-arazzo)
- Spectral CLI: VERIFIED (https://github.com/stoplightio/spectral)

## Corrections Made:
1. OpenAPI file size: 545KB -> 1.1MB
2. Python script line count: 500+ -> 740+
3. Spectral config file: .spectral.yaml -> arazzo-spectral.yaml
4. Spectral CLI version: ^6.11.0 -> ^6.15.0
5. Arazzo spec version: v1.0.0 -> v1.0.1 (throughout)
6. Spectral config content: Updated to match actual arazzo-spectral.yaml
7. Validation checks: Updated to match actual rules in arazzo-spectral.yaml
