...
JWT access tokens expire after 1 hour (configurable via `JWT_EXPIRATION_SECONDS`, default 3600). The refresh token (valid for 30 days) allows obtaining new access tokens without re-authentication.
...
┌─────────────────────────────────────────────────────────────────┐
│ {                                                               │
│   "alg": "HS256",        // Signing algorithm (or RS256/ES256)  │
│   "typ": "JWT",          // Token type                          │
│   "kid": "1"             // Key ID (default "1", configurable)  │
│ }                                                               │
└─────────────────────────────────────────────────────────────────┘
...
VERIFIED CLAIMS:
- PKCE implementation: Confirmed in auth/pkce.go (RFC 7636 compliant)
  - Code verifier: 43-128 characters per MinVerifierLength/MaxVerifierLength constants
  - S256 method only (line 57-58)
  - Constant-time comparison for security (line 65)
- State TTL: 10 minutes confirmed in auth/handlers.go (lines 362, 372, 421, 551, 2076, 2086)
- Access token expiration: Default 3600 seconds (1 hour) - auth/config.go line 156
- Refresh token expiration: 30 days - auth/service.go line 202
- JWT signing methods: HS256, RS256, ES256 supported - auth/config.go lines 503-522
- SAML support: Confirmed - auth/saml/ directory, OpenAPI endpoints /saml/*
- Token blacklist: Confirmed - auth/token_blacklist.go
- OAuth providers: Google, GitHub, Microsoft, TMI test provider - config-example.yml

FILE REFERENCES VERIFIED:
- docs/developer/integration/client-oauth-integration.md - EXISTS
- docs/reference/architecture/README.md - EXISTS
- docs/reference/apis/tmi-openapi.json - EXISTS

CORRECTED:
- Key ID default changed from "tmi-key-1" to "1" (per auth/config.go line 224)
- Added note about configurable JWT expiration
- Added RS256/ES256 as alternative signing algorithms
- Removed link to non-existent oauth-integration.md

EXTERNAL REFERENCES VERIFIED:
- RFC 7636 (PKCE): Verified via IETF/RFC Editor - code_verifier 43-128 chars confirmed
- RFC 6749 (OAuth 2.0): Verified via IETF - authorization code flow confirmed
-->
