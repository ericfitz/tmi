...
<!-- Migrated from: docs/developer/integration/webhook-subscriptions.md on 2025-01-24 -->
...
2. **Event Consumer**: Processes events from Redis Streams and creates delivery records (see `api/webhook_event_consumer.go`)
3. **Challenge Worker**: Verifies new subscriptions using challenge-response protocol (see `api/webhook_challenge_worker.go`)
4. **Delivery Worker**: Delivers webhooks to subscribed endpoints with retries and exponential backoff (see `api/webhook_delivery_worker.go`)
5. **Cleanup Worker**: Removes old delivery records and marks idle/broken subscriptions for deletion (see `api/webhook_cleanup_worker.go`)
6. **Rate Limiter**: Prevents abuse using Redis-based sliding window rate limiting (see `api/webhook_rate_limiter.go`)
...
TMI supports 25 webhook event types covering CRUD operations across 9 resource categories. All event types follow the pattern `{resource}.{action}`.
...
**Note**: All event types are defined in the OpenAPI schema as `WebhookEventType` and in the codebase at `api/events.go`.
...
<!-- Verified against api/webhook_store.go constants -->
- **Max Subscriptions**: 10 active subscriptions
- **Subscription Creation**: 10 requests per minute, 20 requests per day
- **Event Publication**: 12 events per minute
...
<!-- Verified against api/events.go: 10-second TTL with 1-second timestamp window -->
1. **Deduplication**: TMI deduplicates events within 10-second windows (1-second timestamp granularity)
...
<!-- NEEDS-REVIEW: oauth-integration.md not found at expected path; check wiki or migrated folder -->
- [Database Schema](../../reference/schemas/database-schema-complete.md) - Webhook tables
- [Redis Schema](../../operator/database/redis-schema.md) - Redis Streams setup

<!-- Verification Summary:
- File paths verified: api/events.go, api/webhook_*.go files exist
- Event types: 25 total (8 resource categories x 3 CRUD events + 1 addon.invoked) - verified against WebhookEventType enum in OpenAPI spec and api/events.go
- Retry logic: Verified 5 attempts with backoff [1, 5, 15, 30] minutes in api/webhook_delivery_worker.go
- Quotas: Corrected to match api/webhook_store.go constants (10 subs, 10 req/min, 20 req/day, 12 events/min)
- Deduplication: Corrected to 10-second TTL with 1-second window (api/events.go)
- Challenge worker: 30-second interval, max 3 challenges - verified in api/webhook_challenge_worker.go
- Cleanup worker: 1-hour interval, 90-day idle, 10+ failures/7-day broken - verified in api/webhook_cleanup_worker.go
- Delivery worker: 30-second timeout, 2-second check interval - verified in api/webhook_delivery_worker.go
- URL validation: HTTPS required, RFC 1035/1123/5890 hostname validation - verified in api/webhook_url_validator.go
- Metrics: Listed as stubs in documentation, no actual metrics found in code
- oauth-integration.md: NOT FOUND at ../setup/oauth-integration.md
-->
