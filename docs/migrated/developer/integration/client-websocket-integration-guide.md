...
**Note**: The JWT does not contain a role claim. User permissions (reader/writer/owner) are determined dynamically based on the threat model's authorization list, not from the JWT token.
...
VERIFIED:
- WebSocket endpoint path: /threat_models/{id}/diagrams/{id}/ws (confirmed in tmi-openapi.json and tmi-asyncapi.yml)
- REST API endpoints for collaboration: /me/sessions, /threat_models/{id}/diagrams/{id}/collaborate (POST/PUT/GET/DELETE) verified in OpenAPI spec
- Node shape types: actor, process, store, security-boundary, text-box (confirmed in OpenAPI schema)
- Node minimum dimensions: width >= 40, height >= 30 (confirmed in OpenAPI schema)
- Dual format support (flat/nested position) for node coordinates (confirmed in OpenAPI schema)
- Message types: diagram_operation, presenter_request, presenter_cursor, current_presenter, participants_update, participant_joined, participant_left, etc. (confirmed in AsyncAPI spec and websocket_handlers.go)
- Permissions: reader, writer, owner (confirmed in OpenAPI authorization schema)
- Make targets: wstest, build-wstest, clean-wstest (confirmed in Makefile)
- Session states: active, terminating, terminated (confirmed in api/websocket.go)

CORRECTED:
- JWT claims: Removed incorrect "role" claim - JWT does not contain role, permissions are derived from threat model authorization list (verified in auth/service.go Claims struct)

NOT APPLICABLE:
- TMICollaborativeClient class - This is example client code for implementers, not TMI server code
- EventEmitter pattern - Standard JavaScript pattern, not TMI-specific

MIGRATED TO WIKI:
- WebSocket-API-Reference.md already contains equivalent content
- This document provides additional client-side implementation patterns
