...
Auth verification uses the **real Google provider** for interactive sign-in. The deployment runs in **production build mode**, so the tmi provider — though enabled — is restricted to the Client Credentials Grant: `…/oauth2/authorize?idp=tmi&login_hint=…` no longer mints an anonymous JWT (`ExchangeCode` refuses the authorization-code flow in production). Complete an interactive Google OAuth sign-in against `https://server.aws.tmi.dev/oauth2/authorize?idp=google`, obtain a bearer token, then `GET /threat_models`. Confirm the seeded admin (hobobarbarian@gmail.com) has admin capabilities and that a second, non-admin Google account is a reviewer but not admin. As a negative check, confirm `idp=tmi&login_hint=attacker` does NOT yield a usable token. Optional `wstest` against `wss://server.aws.tmi.dev` (using a real token). Extraction smoke: submit a document extraction and verify the extractor TMIComponent scales via KEDA and drains the NATS queue (`kubectl logs -l app=tmi-extractor -n tmi-platform`); this exercises the `tmi-embedding` secret path (Step 1 must have set `TMI_EMBEDDING_API_KEY`).
