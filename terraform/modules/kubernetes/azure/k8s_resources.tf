# Kubernetes Resources for TMI on AKS
# Manages Deployments, Services, ConfigMaps, Secrets, and NGINX Ingress
...
resource "kubernetes_secret_v1" "tmi" {
  ...
  data = {
    TMI_DATABASE_URL            = "postgres://${var.db_username}:${urlencode(var.db_password)}@${var.db_host}:5432/${var.db_name}?sslmode=require"
...
# TMI API ClusterIP Service (NGINX Ingress routes to this)
...
# NGINX Ingress for TMI API with WebSocket support
resource "kubernetes_ingress_v1" "tmi_api" {
  metadata {
    ...
    annotations = {
      ...
      "nginx.ingress.kubernetes.io/websocket-services" = "tmi-api"
