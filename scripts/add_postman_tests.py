...
def add_tests_to_request(request_item):
    ...
    test_event = next(
        (event for event in request_item["event"] if event.get("listen") == "test"),
        None,
    )
    ...
    if test_event is None:
        ...
        # Ensure script dict exists and has correct structure
        if "script" not in test_event:
            test_event["script"] = {"exec": [], "type": "text/javascript"}
        elif not isinstance(test_event["script"].get("exec"), list):
            test_event["script"]["exec"] = []
    ...
    if expected_statuses:
        ...
        test_scripts.append('pm.test("Status code is valid", function () {')
        test_scripts.append(
            f"    pm.expect(pm.response.code).to.be.oneOf([{status_list}]);"
        )
        test_scripts.append("});")
    ...
    # Update the test script - script dict is guaranteed to exist from initialization above
    script = test_event["script"]
    if isinstance(script, dict):
        script["exec"] = test_scripts
