...
def validate_basic_structure(spec: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    if not openapi_version.startswith('3.'):
        warnings.append(f'OpenAPI version {openapi_version} may not be supported (expected 3.x)')
    ...
    for path, path_item in paths.items():
        ...
        for method, operation in path_item.items():
            ...
            if responses:
                ...
                has_redirect = any(str(code).startswith('3') for code in responses.keys())
                ...
                if not has_redirect:
                    has_error = any(str(code).startswith(('4', '5')) for code in responses.keys())
                    if not has_error:
                        warnings.append(f'No error response (4xx/5xx) in {operation_id}')
    ...
    for ref_path, ref in all_refs:
        if ref.startswith('#/components/schemas/'):
            ...
            if scheme_name not in security_schemes:
                errors.append(f'Broken security scheme reference: {ref} used in {ref_path}')
...
def main():
    ...
    parser.add_argument(
        'file', 
        help='OpenAPI specification file (JSON format)'
    )
    parser.add_argument(
        '--quiet', '-q',
        action='store_true',
    ...
    # Final status
    if all_errors:
        print(f'❌ Validation failed with {len(all_errors)} errors and {len(all_warnings)} warnings')
        sys.exit(1)
    elif all_warnings:
