...
Run with: uv run scripts/validate-asyncapi.py tmi-asyncapi.yml
"""

import sys
import yaml  # pyright: ignore[reportMissingModuleSource]  # ty:ignore[unresolved-import]
from typing import Dict, Any, List, Optional
from pydantic import BaseModel, Field, field_validator  # ty:ignore[unresolved-import]
from jsonschema import Draft7Validator  # pyright: ignore[reportMissingModuleSource]  # ty:ignore[unresolved-import]
from jsonschema.exceptions import SchemaError  # pyright: ignore[reportMissingModuleSource]  # ty:ignore[unresolved-import]
import argparse
                    ...
                    if isinstance(msg_ref, dict) and "$ref" in msg_ref:
                        ...
                        if ref_path.startswith("#/components/messages/"):
                            ...
                            if msg_name not in defined_messages:
                                issues.append(
                                    f"Channel '{channel_name}' references undefined message '{msg_name}'"
                                )
                ...
                for msg_ref in op_def["messages"]:
                    if isinstance(msg_ref, dict) and "$ref" in msg_ref:
                        ...
                        if ref_path.startswith("#/components/messages/"):
                            ...
                            if msg_name not in defined_messages:
                                issues.append(
                                    f"Operation '{op_name}' references undefined message '{msg_name}'"
                                )
                ...
                for security_req in server_def["security"]:
                    for scheme_name in security_req.keys():
                        if scheme_name not in defined_schemes:
                            issues.append(
                                f"Server '{server_name}' references undefined security scheme '{scheme_name}'"
                            )
