...
# Enhanced TMI Docker Container Setup Script with Security Scanning
# This script creates secure containers with automated vulnerability patching

# --- Configuration ---
PG_CONTAINER_NAME="tmi-postgresql-secure"
REDIS_CONTAINER_NAME="tmi-redis-secure"
PG_IMAGE="tmi/tmi-postgresql-secure:latest"
REDIS_IMAGE="tmi/tmi-redis-secure:latest"
PG_HOST_PORT="5432"
...
echo "--- TMI Secure Docker Container Setup Script ---"
...
# --- Build Secure Images First ---
log_info "Building secure container images..."

# Check if secure build script exists
if [ -f "scripts/build-secure-containers.sh" ]; then
    log_info "Running secure container build process..."
    ./scripts/build-secure-containers.sh
    if [ $? -ne 0 ]; then
        log_error "Failed to build secure containers. Exiting."
        exit 1
    fi
else
    log_warning "Secure build script not found. Building images with existing Dockerfiles..."
    
    # Build PostgreSQL secure image if Dockerfile exists
    if [ -f "Dockerfile.postgres.secure" ]; then
        log_info "Building PostgreSQL secure image..."
        docker build -f Dockerfile.postgres.secure -t "$PG_IMAGE" .
    else
        log_warning "Using original PostgreSQL image (not patched)"
        PG_IMAGE="bitnami/postgresql:latest"
    fi
    
    # Build Redis secure image if Dockerfile exists
    if [ -f "Dockerfile.redis.secure" ]; then
        log_info "Building Redis secure image..."
        docker build -f Dockerfile.redis.secure -t "$REDIS_IMAGE" .
    else
...
# --- Create tmi-postgresql-secure Container ---
echo ""
log_info "--- Creating tmi-postgresql-secure container ---"
echo "Image: ${PG_IMAGE}"
...
# --- Create tmi-redis-secure Container ---
echo ""
log_info "--- Creating tmi-redis-secure container ---"
