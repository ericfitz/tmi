...
#
# Usage: ./scripts/heroku-reset-database.sh [app-name]
# Example: ./scripts/heroku-reset-database.sh tmi-server
...
heroku run -a "$APP_NAME" 'echo "DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT ALL ON SCHEMA public TO postgres; GRANT ALL ON SCHEMA public TO public;" | psql $DATABASE_URL' 2>&1 | grep -E "(DROP|CREATE|GRANT|ERROR|drop cascades)" || true
...
MIGRATION_OUTPUT=$(heroku run -a "$APP_NAME" './bin/server migrate' 2>&1)
