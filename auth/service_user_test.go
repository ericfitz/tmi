...
func TestUser_Struct(t *testing.T) {
	t.Run("user struct fields are accessible", func(t *testing.T) {
		...
		assert.Equal(t, "user@example.com", user.ProviderUserID)
...
func TestClientCredential_Struct(t *testing.T) {
	t.Run("client credential struct fields are accessible", func(t *testing.T) {
		...
		assert.Equal(t, "A test client credential", cred.Description)
		assert.NotEqual(t, uuid.Nil, cred.OwnerUUID)
		assert.Equal(t, "$2a$10$hash_here", cred.ClientSecretHash)
		assert.True(t, cred.IsActive)
