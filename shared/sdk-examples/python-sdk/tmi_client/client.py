...
class TMIClient:
    ...
    def get_auth_providers(self) -> List[Dict[str, Any]]:
        ...
        response = self._get('/auth/providers')
    ...
    def list_threat_models(self, limit: Optional[int] = None, offset: Optional[int] = None) -> List[ThreatModel]:
    ...
    def create_threat_model(self, threat_model: Union[ThreatModel, Dict[str, Any]]) -> ThreatModel:
        """Create a new threat model."""
        data = threat_model.to_dict() if isinstance(threat_model, ThreatModel) else threat_model
        response = self._post('/threat_models', json=data)
        return ThreatModel.from_dict(response.json())
    
    def update_threat_model(self, threat_model_id: str, threat_model: Union[ThreatModel, Dict[str, Any]]) -> ThreatModel:
        """Update a threat model."""
        data = threat_model.to_dict() if isinstance(threat_model, ThreatModel) else threat_model
        response = self._put(f'/threat_models/{threat_model_id}', json=data)
        return ThreatModel.from_dict(response.json())
    
    def patch_threat_model(self, threat_model_id: str, patches: List[Dict[str, Any]]) -> ThreatModel:
    ...
    def create_diagram(self, threat_model_id: str, diagram: Union[Diagram, Dict[str, Any]]) -> Diagram:
    ...
    def update_diagram(self, threat_model_id: str, diagram_id: str, diagram: Union[Diagram, Dict[str, Any]]) -> Diagram:
        """Update a diagram."""
        data = diagram.to_dict() if isinstance(diagram, Diagram) else diagram
        response = self._put(f'/threat_models/{threat_model_id}/diagrams/{diagram_id}', json=data)
        return Diagram.from_dict(response.json())
    
    def patch_diagram(self, threat_model_id: str, diagram_id: str, patches: List[Dict[str, Any]]) -> Diagram:
        """Apply JSON Patch operations to a diagram."""
        response = self._patch(f'/threat_models/{threat_model_id}/diagrams/{diagram_id}', json=patches)
        return Diagram.from_dict(response.json())
    ...
    def get_collaboration_status(self, threat_model_id: str, diagram_id: str) -> Dict[str, Any]:
        """Get collaboration status for a diagram."""
        response = self._get(f'/threat_models/{threat_model_id}/diagrams/{diagram_id}/collaborate')
        return response.json()
    
    def start_collaboration(self, threat_model_id: str, diagram_id: str) -> Dict[str, Any]:
        """Start a collaboration session for a diagram."""
        response = self._post(f'/threat_models/{threat_model_id}/diagrams/{diagram_id}/collaborate')
        return response.json()
    
    def end_collaboration(self, threat_model_id: str, diagram_id: str) -> None:
        """End a collaboration session for a diagram."""
        self._delete(f'/threat_models/{threat_model_id}/diagrams/{diagram_id}/collaborate')
    ...
    def create_threat(self, threat_model_id: str, threat: Union[Threat, Dict[str, Any]]) -> Threat:
        """Create a new threat."""
    ...
    def update_threat(self, threat_model_id: str, threat_id: str, threat: Union[Threat, Dict[str, Any]]) -> Threat:
        """Update a threat."""
        data = threat.to_dict() if isinstance(threat, Threat) else threat
        response = self._put(f'/threat_models/{threat_model_id}/threats/{threat_id}', json=data)
        return Threat.from_dict(response.json())
    
    def patch_threat(self, threat_model_id: str, threat_id: str, patches: List[Dict[str, Any]]) -> Threat:
        """Apply JSON Patch operations to a threat."""
        response = self._patch(f'/threat_models/{threat_model_id}/threats/{threat_id}', json=patches)
        return Threat.from_dict(response.json())
    ...
    def create_threats_bulk(self, threat_model_id: str, threats: List[Union[Threat, Dict[str, Any]]]) -> List[Threat]:
        """Create multiple threats in bulk."""
        data = [threat.to_dict() if isinstance(threat, Threat) else threat for threat in threats]
        response = self._post(f'/threat_models/{threat_model_id}/threats/bulk', json=data)
        return [Threat.from_dict(threat) for threat in response.json()]
    
    def update_threats_bulk(self, threat_model_id: str, threats: List[Union[Threat, Dict[str, Any]]]) -> List[Threat]:
        """Update multiple threats in bulk."""
        data = [threat.to_dict() if isinstance(threat, Threat) else threat for threat in threats]
        response = self._put(f'/threat_models/{threat_model_id}/threats/bulk', json=data)
        return [Threat.from_dict(threat) for threat in response.json()]
    
    def delete_threats_batch(self, threat_model_id: str, threat_ids: List[str]) -> Dict[str, Any]:
        """Delete multiple threats in a batch."""
        response = self._delete(f'/threat_models/{threat_model_id}/threats/batch', json={"threat_ids": threat_ids})
        return response.json()
    
    def patch_threats_batch(self, threat_model_id: str, patches: List[Dict[str, Any]]) -> List[Threat]:
        """Apply patches to multiple threats in a batch."""
        response = self._post(f'/threat_models/{threat_model_id}/threats/batch/patch', json=patches)
        return [Threat.from_dict(threat) for threat in response.json()]
    ...
    def get_document(self, threat_model_id: str, document_id: str) -> Document:
        ...
        response = self._get(f'/threat_models/{threat_model_id}/documents/{document_id}')
        return Document.from_dict(response.json())
    
    def create_document(self, threat_model_id: str, document: Union[Document, Dict[str, Any]]) -> Document:
    ...
    def update_document(self, threat_model_id: str, document_id: str, document: Union[Document, Dict[str, Any]]) -> Document:
        """Update a document."""
        data = document.to_dict() if isinstance(document, Document) else document
        response = self._put(f'/threat_models/{threat_model_id}/documents/{document_id}', json=data)
        return Document.from_dict(response.json())
    ...
    def create_documents_bulk(self, threat_model_id: str, documents: List[Union[Document, Dict[str, Any]]]) -> List[Document]:
        """Create multiple documents in bulk."""
        data = [doc.to_dict() if isinstance(doc, Document) else doc for doc in documents]
        response = self._post(f'/threat_models/{threat_model_id}/documents/bulk', json=data)
        return [Document.from_dict(doc) for doc in response.json()]
    ...
    def create_source(self, threat_model_id: str, source: Union[Source, Dict[str, Any]]) -> Source:
        """Create a new source."""
    ...
    def update_source(self, threat_model_id: str, source_id: str, source: Union[Source, Dict[str, Any]]) -> Source:
        """Update a source."""
        data = source.to_dict() if isinstance(source, Source) else source
        response = self._put(f'/threat_models/{threat_model_id}/sources/{source_id}', json=data)
        return Source.from_dict(response.json())
    ...
    def create_sources_bulk(self, threat_model_id: str, sources: List[Union[Source, Dict[str, Any]]]) -> List[Source]:
        """Create multiple sources in bulk."""
        data = [source.to_dict() if isinstance(source, Source) else source for source in sources]
        response = self._post(f'/threat_models/{threat_model_id}/sources/bulk', json=data)
        return [Source.from_dict(source) for source in response.json()]
    ...
    def update_metadata_bulk(self, path: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
