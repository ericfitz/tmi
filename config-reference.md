...
<!-- GENERATED by `make generate-config-docs` on 2026-09-05T03:16:00Z — do not edit by hand. -->
...
| `TMI_CONTENT_OAUTH_PROVIDERS_<ID>_<FIELD>` | server | Per-provider content OAuth config, discovered from the _ENABLED field. FIELD is one of CLIENT_ID, CLIENT_SECRET (secret), AUTH_URL, TOKEN_URL, USERINFO_URL, REVOCATION_URL, REQUIRED_SCOPES | no |
| `SAML_PROVIDERS_<ID>_<FIELD>` | server | Per-provider SAML config, discovered from the _ENABLED field; note there is no TMI_ prefix. FIELD is a SAMLProviderConfig yaml key upper-cased, e.g. ENTITY_ID, ACS_URL, SP_PRIVATE_KEY (secret), IDP_METADATA_B64XML (secret) | no |
